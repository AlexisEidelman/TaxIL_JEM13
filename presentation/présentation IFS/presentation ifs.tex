...
\framesubtitle{\vspace{0.3cm} Evaluation inequalities} 
Annual income doesn't measure utility (or living standard) \pause
	\begin{enumerate}
	\item Income chocks (capital gain, unemployment, child care, bad harvest) \pause
	\item Different period of life (education, retirement) \pause
	\item Wealth (and expected inheritance) can't be ignored \pause \newline
	\end{enumerate} \pause
...
\begin{itemize}
		...
		\	$\longrightarrow$ redistribution \pause
	\end{enumerate}
...
\begin{itemize}
\item How many people are impacted by a reform today and during their whole life ? \pause
\item Intergenerational inequalities ?\\	
		\ 	$\longrightarrow$ Life was easier for my parents (no unemployment, lower house pricing, pension without having contributed) \\
		\ 	$\longrightarrow$ Young people are privileged (growth and technology, longer education, longer lives) 
\end{itemize}
\end{frame} 
...
\newline
The aim of \til \ is to be as general as possible and not focused on a specific topic.\\ \pause
...
\begin{itemize}
\item Applied at each period or after the simulation. \pause
\item done with OpenFisca \pause
\begin{itemize}
...
\begin{itemize}
\item with 300,000 individuals (390,000 in all)\\
\	$\longrightarrow$ \ 50 periods run in 39 minutes  \\
\	 = less than a minute per period with an IFS computer
\item It allows to look forward
\end{itemize}
\end{frame}
...
\item \textbf{How \til \ deals with:}
	\begin{itemize}
	\item French wealth survey (2009) $\longrightarrow$ partial profile career 
	\item Administrative data (pension) $\longrightarrow$ complete profile career  \pause
	\item $\Rightarrow$ Statistical survey matching \pause
	\begin{itemize}
...
\begin{frame}
\frametitle{Marriage}
Probabilistic based modelling or behavioural equations model \pause
\begin{itemize}
...
\item \textbf{How \til \ deals with:}  
	\begin{itemize}
	\item Income tax split between spouses...
	\item ...except tax reduction related to children which is split between children.
...
\begin{itemize}
\item \textbf{Issue: } A trade-off between precision and calculus time.  \pause
\item \textbf{How \til \ deals with:} 
	\begin{itemize}
...
\item \textbf{How to get better: }
	\begin{itemize}
	\item Month scale
	\item ...or at least quarter scale  \pause
	\item Simulate\textbf{ durations more than transitions}
	\end{itemize}
\end{itemize}
\end{frame}
...
\item \textbf{How \til \ deals with:} 
	\begin{itemize}
	  ...
	  \item No real other option \pause
	\end{itemize}
\item \textbf{How to get better: } 
	\begin{itemize}
...
\begin{frame}
  ...
  \item \textbf{Langages}
    ...
    \begin{itemize}
		...
		\item Way much faster than statistical language. \pause
    \end{itemize}
...
\begin{frame}[plain,t]
\frametitle{struggling with a python...}
\begin{center}
...
Legislation and population of 2009
	\begin{itemize}
...
\begin{itemize}
\item OpenFisca can deal with other countries legislation \\ \pause
